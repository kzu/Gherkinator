    ...
    public class RunnerTests
    {
        ...
        public void when_adding_fallback_then_succeeds()
            => Scenario(nameof(Scenarios.RunnerScenario))
                ...
                .Run()
                .Verify(state => Assert.All(state.Scenario.Steps, step => Assert.Equal(step.Text, state.Get<string>(step.Text))));
        ...
        public void when_adding_before_after_then_succeeds()
            => Scenario(nameof(Scenarios.RunnerScenario))
                ...
                .Run()
                .Verify(state => Assert.NotNull(state.Get(SdkExtensions.BeforeGiven)))
                .Verify(state => Assert.NotNull(state.Get(SdkExtensions.AfterGiven)))
                .Verify(state => Assert.NotNull(state.Get(SdkExtensions.BeforeWhen)))
                .Verify(state => Assert.NotNull(state.Get(SdkExtensions.AfterWhen)))
                .Verify(state => Assert.NotNull(state.Get(SdkExtensions.BeforeThen)))
                .Verify(state => Assert.NotNull(state.Get(SdkExtensions.AfterThen)));
